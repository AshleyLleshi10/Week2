    ...
    public:
        int getAzimuth (std::vector<std::string> instructions[]){
            ...
            for (int i = 0; i <= instructions->size(); i++)
            {
                ...
                else if (instructions[i].size() == 4 && instructions[i][0] == 'L')
                {
                    //std::cout << "ping left" << std::endl;   
                    azimuth = azimuth - 90; 
                }
                else if (instructions[i].size() == 11)
                {
                    //std::cout << "ping 180" << std::endl;   
                    azimuth = azimuth + 180;
                }
                else if (instructions[i].size() == 4 && instructions[i][0] == 'H')
                {
                    //std::cout << "ping stop" << std::endl;   
                    break;
                }
                else if (instructions[i].size() >= 6 && instructions[i][0] == 'L')
                {
                    //std::cout << "ping left x" << std::endl;   
                    azimuth = azimuth - stoi(instructions[i].erase(0,5));
                }
                else if (instructions[i].size() >= 7 && instructions[i][0] == 'R')
                {
